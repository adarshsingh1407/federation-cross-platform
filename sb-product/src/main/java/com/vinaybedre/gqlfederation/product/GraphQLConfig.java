...
public class GraphQLConfig {
    ...
    public GraphQLSchema customSchema(SchemaParser schemaParser, ProductService productService) {
        return Federation.transform(schemaParser.makeExecutableSchema()).fetchEntities(
                env -> env.<List<Map<String, Object>>>getArgument(_Entity.argumentName).stream().map(values -> {
